...
uses
  ...
  CastleColors, BCButtonFocus, BCButton,
  // editor units
...
type
  ...
  TabDesignFrame = class(TFrame)
    ButtonInteractMode: TBCButtonFocus;
    ButtonTransformRotateMode: TBCButtonFocus;
    ButtonTransformScaleMode: TBCButtonFocus;
    ButtonTransformSelectMode: TBCButtonFocus;
    ButtonTransformTranslateMode: TBCButtonFocus;
    ButtonModifyUiMode: TBCButtonFocus;
    ButtonResetTransformation: TButton;
    ButtonClearAnchorDeltas: TButton;
    ButtonViewportMenu: TBCButtonFocus;
...
{ TabDesignFrame.TDesignerLayer ------------------------------------------------ }

constructor TabDesignFrame.TDesignerLayer.Create(AOwner: TComponent);
begin
...
function TabDesignFrame.TDesignerLayer.HoverUserInterface(
  ...
  begin
    ...
    begin
      for I := C.ControlsCount - 1 downto 0 do
        if TabDesignFrame.Selectable(C.Controls[I]) then
        begin
...
function TabDesignFrame.TDesignerLayer.HoverTransform(
...
function TabDesignFrame.TDesignerLayer.IsResizing(const UI: TCastleUserInterface;
...
function TabDesignFrame.TDesignerLayer.Press(
...
function TabDesignFrame.TDesignerLayer.Release(const Event: TInputPressRelease): Boolean;
var
  Sel: TComponent;
begin
  Result := inherited Press(Event);
...
function TabDesignFrame.TDesignerLayer.Motion(const Event: TInputMotion): Boolean;
...
procedure TabDesignFrame.TDesignerLayer.Render;
...
constructor TabDesignFrame.Create(TheOwner: TComponent);
...
destructor TabDesignFrame.Destroy;
begin
  FreeAndNil(TreeNodeMap);
  inherited Destroy;
end;

procedure TabDesignFrame.SaveDesign(const Url: String);
begin
...
procedure TabDesignFrame.ClearDesign;
begin
...
procedure TabDesignFrame.PerformUndoRedo(const UHE: TUndoHistoryElement);
...
procedure TabDesignFrame.PerformRedo;
begin
  PerformUndoRedo(UndoSystem.Redo);
end;

procedure TabDesignFrame.PerformUndo;
begin
...
procedure TabDesignFrame.UpdateObjectInspectorTimer(Sender: TObject);
//var
//  InspectorType: TInspectorType;
begin
...
procedure TabDesignFrame.OpenDesign(const NewDesignRoot, NewDesignOwner: TComponent;
  const NewDesignUrl: String);
var
...
procedure TabDesignFrame.OpenDesign(const NewDesignUrl: String);
var
...
function TabDesignFrame.FormCaption: String;
var
  DesignName: String;
begin
...
procedure TabDesignFrame.BeforeProposeSaveDesign;
var
  InspectorType: TInspectorType;
begin
...
procedure TabDesignFrame.AddComponent(const ComponentClass: TComponentClass;
  const ComponentOnCreate: TNotifyEvent);
var
...
function TabDesignFrame.AddComponent(const ParentComponent: TComponent;
...
procedure TabDesignFrame.DeleteComponent;
...
procedure TabDesignFrame.CopyComponent;
var
  Sel: TComponent;
begin
...
procedure TabDesignFrame.PasteComponent;
...
procedure TabDesignFrame.DuplicateComponent;
...
function TabDesignFrame.SelectedViewport: TCastleViewport;
var
  Selected: TComponentList;
...
procedure TabDesignFrame.UIScaling(const UIScaling: TUIScaling;
  const UIReferenceWidth, UIReferenceHeight: Single);
begin
...
procedure TabDesignFrame.CurrentComponentApiUrl(var Url: String);
...
function TabDesignFrame.ComponentCaption(const C: TComponent): String;
...
procedure TabDesignFrame.CastleControlResize(Sender: TObject);
var
...
procedure TabDesignFrame.CastleControlOpen(Sender: TObject);
...
procedure TabDesignFrame.CastleControlUpdate(Sender: TObject);
begin
...
procedure TabDesignFrame.CastleControlDragOver(Sender, Source: TObject; X,
  ...
  begin
    ...
    { ShellList.Selected may be nil, testcase:
      ...
      - mouse over the design -> without this check, would have access violation
        due to TabDesignFrame.CastleControlDragOver being called with
...
procedure TabDesignFrame.CastleControlDragDrop(Sender, Source: TObject; X, Y: Integer);
var
  Viewport: TCastleViewport;
...
procedure TabDesignFrame.InspectorFilter(Sender: TObject;
  AEditor: TPropertyEditor; var AShow: Boolean; const Section: TPropertySection);
var
  PropertyName: String;
...
procedure TabDesignFrame.GizmoHasModifiedParent(Sender: TObject);
begin
...
procedure TabDesignFrame.GizmoStopDrag(Sender: TObject);
begin
...
procedure TabDesignFrame.InspectorBasicFilter(Sender: TObject;
  AEditor: TPropertyEditor; var aShow: Boolean);
begin
  InspectorFilter(Sender, AEditor, AShow, psBasic);
end;

procedure TabDesignFrame.InspectorLayoutFilter(Sender: TObject;
  AEditor: TPropertyEditor; var aShow: Boolean);
begin
  InspectorFilter(Sender, AEditor, AShow, psLayout);
end;

function TabDesignFrame.UndoMessageModified(const Sel: TPersistent;
  const ModifiedProperty, ModifiedValue: String; const SelectedCount: Integer): String;
const
...
procedure TabDesignFrame.PropertyGridModified(Sender: TObject);
...
procedure TabDesignFrame.PropertyEditorModified(Sender: TObject);
var
  Sel: TPersistent;
begin
...
procedure TabDesignFrame.RecordUndo(const UndoComment: String;
  const UndoCommentPriority: TUndoCommentPriority; const ItemIndex: Integer = -1);
var
...
procedure TabDesignFrame.MarkModified;
begin
  // mark modified
...
class function TabDesignFrame.Selectable(const Child: TComponent): Boolean;
begin
...
function TabDesignFrame.Deletable(const Child: TComponent): Boolean;
begin
...
procedure TabDesignFrame.UpdateDesign;
...
function TabDesignFrame.SelectedFromNode(Node: TTreeNode): TComponent;
begin
...
procedure TabDesignFrame.GetSelected(out Selected: TComponentList;
  out SelectedCount: Integer);
...
function TabDesignFrame.GetSelectedUserInterface: TCastleUserInterface;
var
  C: TComponent;
begin
...
procedure TabDesignFrame.SetSelectedUserInterface(const Value: TCastleUserInterface);
begin
  SelectedComponent := Value;
end;

function TabDesignFrame.GetSelectedTransform: TCastleTransform;
var
  C: TComponent;
begin
  C := GetSelectedComponent;
...
procedure TabDesignFrame.SetSelectedTransform(const Value: TCastleTransform);
begin
  SelectedComponent := Value;
end;

function TabDesignFrame.GetSelectedComponent: TComponent;
...
procedure TabDesignFrame.SetSelectedComponent(const Value: TComponent);
var
  Node: TTreeNode;
begin
  if Value = nil then
...
procedure TabDesignFrame.UpdateSelectedControl;
var
  Selected: TComponentList;
...
procedure TabDesignFrame.ControlsTreeSelectionChanged(Sender: TObject);
begin
  UpdateSelectedControl;
end;

procedure TabDesignFrame.ControlsTreeDragOver(Sender, Source: TObject; X,
  Y: Integer; State: TDragState; var Accept: Boolean);
...
procedure TabDesignFrame.ControlsTreeEditing(Sender: TObject; Node: TTreeNode;
  var AllowEdit: Boolean);
var
  C: TComponent;
begin
...
procedure TabDesignFrame.ControlsTreeEditingEnd(Sender: TObject; Node: TTreeNode;
  Cancel: Boolean);
var
  UndoComment: String;
  Sel: TComponent;
...
function TabDesignFrame.ControlsTreeAllowDrag(const Src, Dst: TTreeNode): Boolean;
var
...
procedure TabDesignFrame.FrameAnchorsChange(Sender: TObject);
var
  UI: TCastleUserInterface;
...
procedure TabDesignFrame.AdjustUserInterfaceAnchorsToKeepRect(
...
procedure TabDesignFrame.ControlsTreeEndDrag(Sender, Target: TObject; X,
  Y: Integer);
begin
  ControlsTreeNodeUnderMouse := nil;
  ControlsTree.Invalidate; // force custom-drawn look redraw
...
function TabDesignFrame.RenamePossible: Boolean;
begin
...
procedure TabDesignFrame.RenameSelectedItem;
begin
  if RenamePossible then
    ControlsTree.Selected.EditText;
end;

procedure TabDesignFrame.ControlsTreeDragDrop(Sender, Source: TObject; X,
  Y: Integer);
var
  Src, Dst: TTreeNode;
...
procedure TabDesignFrame.ControlsTreeAdvancedCustomDrawItem(
...
procedure TabDesignFrame.CheckParentSelfAnchorsEqualChange(Sender: TObject);
var
  UI: TCastleUserInterface;
  OldRect: TFloatRectangle;
begin
...
procedure TabDesignFrame.ButtonClearAnchorDeltasClick(Sender: TObject);
var
  UI: TCastleUserInterface;
begin
  UI := SelectedUserInterface;
...
procedure TabDesignFrame.ButtonResetTransformationClick(Sender: TObject);
var
  T: TCastleTransform;
begin
  T := SelectedTransform;
...
procedure TabDesignFrame.ButtonTransformRotateModeClick(Sender: TObject);
begin
...
procedure TabDesignFrame.ButtonTransformScaleModeClick(Sender: TObject);
begin
  if InsideToggleModeClick then Exit;
  InsideToggleModeClick := true;
...
procedure TabDesignFrame.ButtonTransformSelectModeClick(Sender: TObject);
begin
  if InsideToggleModeClick then Exit;
  InsideToggleModeClick := true;
...
procedure TabDesignFrame.ButtonTransformTranslateModeClick(Sender: TObject);
begin
  if InsideToggleModeClick then Exit;
  InsideToggleModeClick := true;
...
procedure TabDesignFrame.ButtonViewportMenuClick(Sender: TObject);
begin
  MenuViewport.PopupComponent := ButtonViewportMenu;
  MenuViewport.PopUp;
end;

procedure TabDesignFrame.ButtonInteractModeClick(Sender: TObject);
begin
  if InsideToggleModeClick then Exit;
  InsideToggleModeClick := true;
  ChangeMode(moInteract);
...
procedure TabDesignFrame.ButtonModifyUiModeClick(Sender: TObject);
begin
  if InsideToggleModeClick then Exit;
  InsideToggleModeClick := true;
...
procedure TabDesignFrame.MenuItemAddComponentClick(Sender: TObject);
var
...
procedure TabDesignFrame.MenuTreeViewItemRenameClick(Sender: TObject);
begin
  RenameSelectedItem;
end;

procedure TabDesignFrame.MenuTreeViewItemDeleteClick(Sender: TObject);
begin
  DeleteComponent;
end;

procedure TabDesignFrame.MenuTreeViewItemCopyClick(Sender: TObject);
begin
  CopyComponent;
end;

procedure TabDesignFrame.MenuTreeViewItemPasteClick(Sender: TObject);
begin
  PasteComponent;
end;

procedure TabDesignFrame.MenuTreeViewPopup(Sender: TObject);
var
  Sel: TComponent;
begin
  Sel := SelectedComponent;
...
procedure TabDesignFrame.MenuViewportNavigation2DClick(Sender: TObject);
begin
  ChangeViewportNavigation(TCastle2DNavigation.Create(DesignOwner));
end;

procedure TabDesignFrame.MenuTreeViewItemDuplicateClick(Sender: TObject);
begin
  DuplicateComponent;
end;

procedure TabDesignFrame.MenuItemViewportCamera2DViewInitialClick(
  Sender: TObject);
var
  V: TCastleViewport;
begin
...
procedure TabDesignFrame.MenuItemViewportCameraCurrentFromInitialClick(
  Sender: TObject);
var
  V: TCastleViewport;
begin
...
procedure TabDesignFrame.MenuItemViewportCameraViewAllClick(Sender: TObject);
var
  V: TCastleViewport;
...
procedure TabDesignFrame.MenuItemViewportCameraSetInitialClick(Sender: TObject);
var
  V: TCastleViewport;
...
procedure TabDesignFrame.MenuItemViewportSort2DClick(Sender: TObject);
var
  V: TCastleViewport;
begin
  V := SelectedViewport;
...
{
function TabDesignFrame.SelectionSave: Classes.TList;
var
  I: Integer;
begin
...
procedure TabDesignFrame.SelectionRestoreAndFree(var Selection: Classes.TList);
begin
...
procedure TabDesignFrame.ChangeViewportNavigation(const NewNavigation: TCastleNavigation);
var
  V: TCastleViewport;
begin
  V := SelectedViewport;
...
procedure TabDesignFrame.MenuViewportNavigationNoneClick(Sender: TObject);
begin
  ChangeViewportNavigation(nil);
end;

procedure TabDesignFrame.MenuViewportNavigationExamineClick(Sender: TObject);
begin
  ChangeViewportNavigation(TCastleExamineNavigation.Create(DesignOwner));
end;

procedure TabDesignFrame.MenuViewportNavigationFlyClick(Sender: TObject);
var
...
procedure TabDesignFrame.MenuViewportNavigationWalkClick(Sender: TObject);
var
  W: TCastleWalkNavigation;
begin
  W := TCastleWalkNavigation.Create(DesignOwner);
...
procedure TabDesignFrame.MenuViewportNavigationThirdPersonClick(Sender: TObject);
var
...
procedure TabDesignFrame.SetParent(AParent: TWinControl);
{$ifdef LCLwin32}
var
  H: Integer;
  InspectorType: TInspectorType;
...
function TabDesignFrame.ProposeName(const ComponentClass: TComponentClass;
  const ComponentsOwner: TComponent): String;
...
procedure TabDesignFrame.UpdateLabelSizeInfo(const UI: TCastleUserInterface);
var
  RR: TFloatRectangle;
  S: String;
begin
...
procedure TabDesignFrame.UpdateAnchors(const UI: TCastleUserInterface;
  const AllowToHideParentAnchorsFrame: Boolean);
begin
...
procedure TabDesignFrame.ChangeMode(const NewMode: TMode);
begin
  Mode := NewMode;
...
procedure TabDesignFrame.ModifiedOutsideObjectInspector(const UndoComment: String;
  const UndoCommentPriority: TUndoCommentPriority; const UndoOnRelease: Boolean = false);
var
  InspectorType: TInspectorType;
begin
...
procedure TabDesignFrame.NewDesign(const ComponentClass: TComponentClass;
  const ComponentOnCreate: TNotifyEvent);
var
  NewRoot: TComponent;
  NewDesignOwner: TComponent;
